...
//문제 설명
//"온라인 결제시스템을 개발하는 개발자가 여러 종류의 신용카드를 지원하고 싶어한다. 
//하지만 카드번호가 길어서 입력하다 실수하는 경우가 잦으므로 사용자가 입력한 번호를 확인할 방법이 필요하다.
//
//개발자는 다음과 같은 "룬 알고리즘"이 모든 카드에 적용됨을 알고 있다.
//먼저 각 신용카드의 각 숫자를 쪼갠다. 예를 들어서, 21378은
//2 1 3 7 8
//
//만약 짝수 개수의 숫자가 존재한다면 홀수의 위치에 있는 숫자에 2를 곱한다. 
//반대로 홀수 개수의 숫자가 존재한다면 짝수의 위치에 있는 숫자에 2를 곱한다. 위치를 구할 때는 첫번째 숫자를 1로 지정한다. 
//위에 예시로 든 번호는 홀수 개수의 숫자가 있으므로 짝수의 위치에 있는 숫자에 2를 곱한다:
//
//2 1 3 7 8 은
//2 2 3 14 8 이 된다.
//
...
//
//신용카드번호 String cardNumber에 대하여 카드번호가 유효하다면 "VALID", 그렇지 않다면 "INVALID"를 반환하라. 
//(따옴표는 문자열을 나타내기 위함일 뿐이며 결과값에는 포함되지 않는다)"
//
//참고 / 제약 사항
		 ...
		 int size = cardNumber.length();
	        ...
	        for(int i=0; i<size; i++){
	            ...
	            if(i%2==odd){
	                ...
	                if(oddn>=10){
	                    sum += oddn%10;
	                    System.out.println(oddn + " " + oddn%10);
